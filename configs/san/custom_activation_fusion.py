# ============================================================
# Custom Activation Fusion SAN ViT-L/14 Config for PlantSeg115
# ============================================================
...
train_dataloader = dict(
    batch_size=1,  # Single GPU memory constraint
    num_workers=2,
...
# --------------- Model Settings (ViT-L/14 + Custom Activation) ---------------
...
model = dict(
    ...
    decode_head=dict(
        ...
        san_cfg=dict(
            ...
            # Custom Activation Fusion Parameters
            init_tau=1.0,      # Temperature τ (learnable)
            init_beta=0.1,     # Difference weight β (learnable)
            init_alpha=0.5,    # Output scaling α (learnable)
...
optim_wrapper = dict(
    ...
    accumulative_counts=4,  # Effective batch size = 4
    optimizer=dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            ...
            # Custom fusion learnable parameters
            'tau': dict(lr_mult=1.0, decay_mult=0.),
            'beta': dict(lr_mult=1.0, decay_mult=0.),
            'alpha': dict(lr_mult=1.0, decay_mult=0.),
        }),
    loss_scale='dynamic',
